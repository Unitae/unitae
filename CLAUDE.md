...
**Auth model:** `Permission` (24 entries, in `app/shared/types/permission.ts`) is the unit of access. **Roles** (DB table) bundle permissions and are assigned to users. Three kinds: **identity** roles synced from `Member` flags (`BUILT_IN_ROLE_KEYS`), **system** roles granted by hand (`SYSTEM_ROLE_KEYS`, currently just `admin`), and **custom** roles a Roles Manager creates. Identity and system roles both carry `isBuiltIn` — use `isIdentityRoleKey()` when the question is which side a role attaches to, since `isBuiltIn` only means "undeletable". A role is the *only* way a permission reaches an account: the direct `CongregationUserPermission` edge was dropped in #149, so never reintroduce a user→permission write. `requireAuth()` runs `resolveEffectivePermissions` and stores the user's full granted set in `permissionsContext`; the legacy `_required` parameter is retained for call-site compatibility but no longer filters anything. See `docs/development/permissions-and-roles.md`.
