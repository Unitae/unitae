...
import {
  ...
  DynamicType,
} from "~/features/board/model/dynamic-document.type";
import { listAvailableDynamicTypes } from "~/features/board/server/dynamic-documents.server";
...
export default function NewDynamicDocumentPage({
  loaderData,
    ...
    <div className="flex flex-col gap-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground text-sm">
              {m.board_new_dynamic_requires_section()}
            </p>
          </CardContent>
        </Card>
      )}
      ...
      {available.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {available.map((item) => (
            <AvailableCard
              key={`${item.dynamicType}|${item.dynamicRef ?? ""}`}
              item={item}
              disabled={!hasSection}
            />
          ))}
        ...
        <div className="flex-1">
          ...
          {item.alreadyAdded && (
            <p className="text-muted-foreground text-xs">
              {m.board_new_dynamic_already_added()}
            </p>
          )}
        </div>
        <Form method="post">
          ...
          <input
            type="hidden"
            name="dynamicRef"
            value={item.dynamicRef ?? ""}
          />
          <input type="hidden" name="title" value={item.defaultTitle} />
          <Button
            type="submit"
            size="sm"
            disabled={item.alreadyAdded || disabled}
          >
...
export async function action({ request }: Route.ActionArgs) {
  const { session, can, congregationId } = await authenticateAndAuthorize(
    request,
    [Role.BoardValidator],
  );
