  ...
  return withScopeFromContext(context, async (db) => {
    ...
    const templates =
      settings.dynamicType === DynamicType.Programme
        ? await getTemplates(db, congregationId)
        : [];
...
export default function EditDynamicDocumentPage({
  loaderData,
  actionData,
  ...
  const [templateConfigs, setTemplateConfigs] = useState<
    Record<number, TemplateConfig>
  >(() => {
    if (existingConfig) {
      const configMap = new Map(
        existingConfig.templates.map((t) => [t.templateId, t]),
      );
    ...
    return Object.fromEntries(
      templates.map((t) => [
        t.id,
        { selected: true, parts: true, services: true },
      ]),
    );
  });

  const [groupBy, setGroupBy] = useState<"date" | "template">(
    existingConfig?.groupBy ?? "date",
  );
  ...
  if (settings.visibleFrom !== null) {
    ...
    visibleFrom.setMinutes(
      visibleFrom.getMinutes() - visibleFrom.getTimezoneOffset(),
    );
  ...
  if (settings.visibleUntil !== null) {
    ...
    visibleUntil.setMinutes(
      visibleUntil.getMinutes() - visibleUntil.getTimezoneOffset(),
    );
      ...
      <PageHeader
        ...
        actions={
          <Button variant="destructive" size="icon" asChild>
            <Link
              to={`/board/dynamic/${settings.id}/delete`}
              title={m.board_dynamic_delete_tooltip()}
            >
          ...
          <CardContent className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor={fields.title.id}>
                {m.board_documents_new_name_label()}
              </Label>
              <Input
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor={fields.sectionId.id}>
                {m.board_documents_new_section_label()}
              </Label>
              <select
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="flex flex-col gap-2">
                <Label htmlFor={fields["visible-from"].id}>
                  {m.board_documents_new_visible_from_label()}
                </Label>
                <Input
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor={fields["visible-until"].id}>
                  {m.board_documents_new_visible_until_label()}
                </Label>
                <Input
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label
                htmlFor={fields.hightlighted.id}
            ...
            <Card>
              <CardContent className="flex flex-col gap-4">
                <h3 className="font-semibold text-sm">
                  {m.board_dynamic_display_options_title()}
                </h3>
                  ...
                  <div className="rounded-md border">
                    <div className="flex items-center gap-3 border-b bg-muted/50 px-3 py-2 text-muted-foreground text-xs font-medium">
                      ...
                      <div className="flex-1">
                        {m.programs_export_meeting_type_label()}
                      </div>
                      <div className="w-20 text-center">
                        {m.programs_export_col_parts()}
                      </div>
                      <div className="w-20 text-center">
                        {m.programs_export_col_services()}
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="date">
                        {m.programs_export_group_by_date()}
                      </SelectItem>
                      <SelectItem value="template">
                        {m.programs_export_group_by_template()}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <input
                  type="hidden"
                  name="dynamicConfig"
                  value={JSON.stringify(dynamicConfigJson)}
                />
              </CardContent>
            </Card>
          )}
...
export async function action({ request, params, context }: Route.ActionArgs) {
  ...
  return withScopeFromContext(context, async (db) => {
    const settings = await updateDynamicDocument(
      db,
      dynamicId,
      congregationId,
      {
    ...
    session.flash(
      "success",
      m.board_dynamic_edit_success({ name: settings.title }),
    );
