...
describe('computeDraftTotals', () => {
  it('sums homes for Classical territories, count is entrance count', () => {
    const result = computeDraftTotals(TerritoryKindKey.Classical, [entrance({ homes: 12 }), entrance({ homes: 8 })])
  ...
  it('for Classical, falls back to phones when homes is zero (mirrors computeTerritoryQuantity)', () => {
    const result = computeDraftTotals(TerritoryKindKey.Classical, [entrance({ homes: 0, phones: 15 })])
    expect(result.primary).toBe(15)
  })

  it('sums phones for Phone territories', () => {
    const result = computeDraftTotals(TerritoryKindKey.Phone, [entrance({ phones: 40 }), entrance({ phones: 25 })])
    expect(result).toEqual({ metric: 'phones', primary: 65, count: 2 })
  })

  it('sums homes-or-phones for Univ (campus) territories', () => {
    const result = computeDraftTotals(TerritoryKindKey.Univ, [
      entrance({ homes: 5 }),
      entrance({ homes: 0, phones: 12 }),
    ])
  ...
  it('reports only entrance count for Commerces (no per-entrance quantity)', () => {
    const result = computeDraftTotals(TerritoryKindKey.Commerces, [entrance(), entrance()])
    expect(result).toEqual({ metric: 'count', primary: 2, count: 2 })
  })

  it('reports only entrance count for Hotel', () => {
    const result = computeDraftTotals(TerritoryKindKey.Hotel, [entrance(), entrance(), entrance()])
    expect(result).toEqual({ metric: 'count', primary: 3, count: 3 })
  })

  it('handles the empty draft cleanly', () => {
    expect(computeDraftTotals(TerritoryKindKey.Classical, [])).toEqual({ metric: 'homes', primary: 0, count: 0 })
    expect(computeDraftTotals(TerritoryKindKey.Commerces, [])).toEqual({ metric: 'count', primary: 0, count: 0 })
  })
})
