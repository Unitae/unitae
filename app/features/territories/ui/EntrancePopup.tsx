...
function actionLabel(entrance: BboxEntrance, pending: EntrancePendingState) {
  ...
  if (entrance.status === "in-this-territory")
    return m.territories_map_action_remove();
...
export default function EntrancePopup({
  ...
  if (confirmingReassign && entrance.otherTerritory != null) {
    return (
      <div className="flex min-w-[280px] max-w-[340px] flex-col gap-2 m-[12px]">
        <div
          className={`-mx-3 -mt-3 mb-1 h-1 rounded-t ${accentClassFor(entrance, pending)}`}
          aria-hidden="true"
        />
        <p className="font-semibold text-base">
          {m.territories_map_reassign_confirm_title()}
        </p>
  ...
  return (
    <div className="flex min-w-[280px] max-w-[340px] flex-col gap-1.5 m-3">
      <div
        className={`-mx-3 -mt-3 mb-1 h-1 rounded-t-lg ${accentClassFor(entrance, pending)}`}
        aria-hidden="true"
      />
      <p className="font-semibold text-base">
        {entrance.address.number} {entrance.address.street},{" "}
        {entrance.address.zip}
      </p>
      <p className="text-muted-foreground text-xs">
        {entranceContentLabel(territoryType, entrance)}
      </p>
      <p className="text-muted-foreground text-xs">
        {statusLine(entrance, pending)}
      </p>
