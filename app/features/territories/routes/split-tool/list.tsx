...
export function loader({ context }: Route.LoaderArgs) {
  ...
  return withScopeFromContext(context, async db => {
    ...
    const [suggestedNumber, fallbackCenter, counts] = await Promise.all([
      computeNextTerritoryNumber(db, congregationId, TerritoryKindKey.Classical),
      getCongregationCenter(db, congregationId),
      countAvailableEntrances(db, congregationId, TerritoryKindKey.Classical, { phoneTypeActive }),
    ])
