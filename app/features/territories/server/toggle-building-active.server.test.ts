...
vi.mock('~/shared/infra/db.server', () => ({
  unscopedDb: { building: { update: vi.fn() } },
}))
...
describe('toggleBuildingActive', () => {
  it('sets active to true', async () => {
    ...
    const result = await toggleBuildingActive(db as never, 1, 1, true)
  ...
  it('sets active to false', async () => {
    ...
    const result = await toggleBuildingActive(db as never, 2, 3, false)
