...
describe('contentPresentClause', () => {
  it('returns null for Commerces territories (prospection alone is enough)', () => {
    expect(contentPresentClause(TerritoryKindKey.Commerces, { phoneTypeActive: true })).toBeNull()
    expect(contentPresentClause(TerritoryKindKey.Commerces, { phoneTypeActive: false })).toBeNull()
  })

  it('returns null for Hotel territories', () => {
    expect(contentPresentClause(TerritoryKindKey.Hotel, { phoneTypeActive: true })).toBeNull()
  })

  it('returns null for Univ territories', () => {
    expect(contentPresentClause(TerritoryKindKey.Univ, { phoneTypeActive: false })).toBeNull()
  })

  it('Classical with phone toggle ON requires homes > 0 or digicode-unknown', () => {
    expect(contentPresentClause(TerritoryKindKey.Classical, { phoneTypeActive: true })).toEqual({
      OR: [{ homes: { gt: 0 } }, digicodeUnknown],
    })
  })

  it('Classical with phone toggle OFF falls back to homes OR phones OR digicode-unknown', () => {
    expect(contentPresentClause(TerritoryKindKey.Classical, { phoneTypeActive: false })).toEqual({
      OR: [{ homes: { gt: 0 } }, { phones: { gt: 0 } }, digicodeUnknown],
    })
  })

  it('Phone requires phones > 0 or digicode-unknown', () => {
    expect(contentPresentClause(TerritoryKindKey.Phone, { phoneTypeActive: true })).toEqual({
...
describe('mapVisibleWhere', () => {
  it('always includes the own-territory branch, keyed by territoryId', () => {
    const result = mapVisibleWhere(TerritoryKindKey.Classical, 42, { phoneTypeActive: true })
  ...
  it('wraps the content clause under a prospected-buildings AND for residential kinds', () => {
    const result = mapVisibleWhere(TerritoryKindKey.Classical, 1, { phoneTypeActive: true })
  ...
  it('drops the content clause for non-residential kinds — prospection alone', () => {
    const result = mapVisibleWhere(TerritoryKindKey.Commerces, 1, { phoneTypeActive: true })
  ...
  it('composes the Classical toggle-off clause into the prospected branch', () => {
    const result = mapVisibleWhere(TerritoryKindKey.Classical, 7, { phoneTypeActive: false })
  ...
  it('composes the Phone clause into the prospected branch', () => {
    const result = mapVisibleWhere(TerritoryKindKey.Phone, 3, { phoneTypeActive: true })
    expect(result).toEqual({
      OR: [
...
describe('availableForCreateWhere', () => {
  ...
  it('excludes entrances already attached to a territory of the same kind', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Commerces, { phoneTypeActive: true })
    expect(result.AND).toContainEqual({ territories: { none: { type: TerritoryKindKey.Commerces } } })
  })
  ...
  it('Classical with phone toggle ON only shows intercom / doorbell / (code + isOpenEarly)', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Classical, { phoneTypeActive: true })
    expect(result).toEqual({
      AND: [
        { territories: { none: { type: TerritoryKindKey.Classical } } },
        prospectedAndActive,
        {
          OR: [
  ...
  it('Classical with phone toggle OFF widens the code branch to any code entrance', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Classical, { phoneTypeActive: false })
  ...
  it('Phone requires phones > 0 or a code-locked entrance that stays locked in the morning', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Phone, { phoneTypeActive: true })
    expect(result).toEqual({
      AND: [
        { territories: { none: { type: TerritoryKindKey.Phone } } },
        prospectedAndActive,
        {
  ...
  it('Phone clause is independent of phoneTypeActive (tab loader gates access to the whole flow)', () => {
    const on = availableForCreateWhere(TerritoryKindKey.Phone, { phoneTypeActive: true })
    const off = availableForCreateWhere(TerritoryKindKey.Phone, { phoneTypeActive: false })
    expect(off).toEqual(on)
  })

  it('Commerces has no access clause — prospection + not-already-typed is enough', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Commerces, { phoneTypeActive: false })
    expect(result).toEqual({
  ...
  it('Hotel has no access clause', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Hotel, { phoneTypeActive: true })
    expect(result).toEqual({
      AND: [{ territories: { none: { type: TerritoryKindKey.Hotel } } }, prospectedAndActive],
    })
  })

  it('Univ has no access clause', () => {
    const result = availableForCreateWhere(TerritoryKindKey.Univ, { phoneTypeActive: false })
    expect(result).toEqual({
      AND: [{ territories: { none: { type: TerritoryKindKey.Univ } } }, prospectedAndActive],
    })
  })
})
