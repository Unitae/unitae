...
export function action({ request, params, context }: Route.ActionArgs) {
  ...
  return withScopeFromContext(context, async db => {
    ...
    if (account == null || account.memberId == null) {
      session.flash('error', m.settings_user_mark_as_left_error({ email: account?.email ?? '' }))
    } else {
      try {
        await setMemberLeft(db, account.memberId, currentUser.congregationId, currentUser.id)
        session.flash('success', m.settings_user_mark_as_left_success({ email: account.email }))
      } catch (error) {
