...
import type { Route } from './+types/import-status'

export const meta: Route.MetaFunction = () => {
  return [{ title: m.import_status_meta_title() }]
}
...
export default function ImportStatusPage({ loaderData }: Route.ComponentProps) {
  const { state, progress, failedReason } = loaderData
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {state === 'completed'
              ? m.import_status_completed()
              : state === 'failed'
                ? m.import_status_failed()
                : m.import_status_title()}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {state !== 'completed' && state !== 'failed' && (
            <>
              ...
              <p className="text-muted-foreground text-sm">
                {progress > 0 ? m.import_status_progress({ progress: String(progress) }) : m.import_status_waiting()}
              </p>
            </>
          )}
          ...
          {state === 'completed' && (
            <>
              <p className="text-sm">{m.import_status_completed_message()}</p>
              <Button asChild>
                <Link to="/settings/congregation">{m.import_status_back()}</Link>
              </Button>
            </>
          )}
