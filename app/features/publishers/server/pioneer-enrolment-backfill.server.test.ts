...
describe('deriveStintsFromActivity', () => {
  it('groups a continuing annual pioneer (spanning the Sept boundary) into one ongoing stint', () => {
    ...
    const stints = deriveStintsFromActivity(rows, PERM)
  ...
  it('leaves a mid-year annual start ongoing from its first reported month', () => {
    const stints = deriveStintsFromActivity([row(1, 0, 2026, PERM), row(2, 1, 2026, PERM)], PERM)
    expect(stints).toEqual([
  ...
  it('closes a concluded annual run at its last served month (member back to Normal)', () => {
    ...
    const stints = deriveStintsFromActivity(rows, NORMAL)
    expect(stints).toEqual([
  ...
  it('does not break a run across a no-row gap', () => {
    ...
    const stints = deriveStintsFromActivity([row(1, 8, 2025, PERM), row(2, 10, 2025, PERM)], PERM)
    expect(stints).toEqual([
  ...
  it('emits one single-month stint per reported month for a MONTHLY auxiliary (member not permanent aux)', () => {
    ...
    const stints = deriveStintsFromActivity([row(1, 9, 2025, AUX), row(2, 11, 2025, AUX)], NORMAL)
    expect(stints).toEqual([
  ...
  it('groups a PERMANENT auxiliary (member.type auxiliaires) into one ongoing stint', () => {
    const stints = deriveStintsFromActivity([row(1, 8, 2025, AUX), row(2, 9, 2025, AUX)], AUX)
    expect(stints).toEqual([
      { type: AUX, startMonth: 8, startYear: 2025, endMonth: null, endYear: null, monthlyGoal: null },
    ])
  })
  ...
  it('returns no stints when the member never had pioneer activity', () => {
    expect(deriveStintsFromActivity([row(1, 8, 2025, NORMAL)], NORMAL)).toEqual([])
    expect(deriveStintsFromActivity([], NORMAL)).toEqual([])
  })
})
