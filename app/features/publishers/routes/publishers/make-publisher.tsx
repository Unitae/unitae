...
export function action({ request, params, context }: Route.ActionArgs) {
  ...
  return withScopeFromContext(context, async db => {
    ...
    if (!member) {
      session.flash('error', m.publishers_view_make_publisher_error({ name }))
    } else {
      const updated = await togglePublisherStatus(db, memberId, currentUser.congregationId, true, currentUser.id)
      if (updated.isPublisher === true) {
        session.flash('success', m.publishers_view_make_publisher_success({ name }))
      } else {
        session.flash('error', m.publishers_view_make_publisher_error({ name }))
      }
    }
